...
$piloto_principal = isset($_POST["piloto_principal"]) ? intval($_POST["piloto_principal"]) : 0;

try {
    $vuelo = new Vuelo();
    $copilotos = $vuelo->pilotoDisponible($fecha_salida, $fecha_llegada);

    if(!empty($copilotos) && is_array($copilotos)) {
        $data = [];
        foreach ($copilotos as $p) {
            if($piloto_principal !== 0 && $p->getId() == $piloto_principal) {
                continue;
            }
        ...
        if(empty($data)) {
            die(json_encode(['error' => 'No hay copilotos disponibles']));
        }
